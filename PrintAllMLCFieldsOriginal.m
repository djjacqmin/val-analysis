...
beamtype =  '1 - Open Field Photons'; % '2 - 60-Degree Wedge Photons'; %'2 - Wedged Field\\';
fieldtype = '05 - MLC Fields'; %'01 - Symmetric Fields\\';
...
Gamma_Map_Points = zeros(length(FieldSize_X)+1,length(Depth));
Gamma_Map_Passing_Points = zeros(length(FieldSize_X)+1,length(Depth));
...
for f = 1:length(FieldSize_X)
    ...
    for d = 1:length(Depth)
        ...
        if ( filename_long_m == 1 )
            filename = sprintf('%s\\%s\\%s\\Mapcheck\\MLC_%s_%s_%dx%d_%dcm_%dMU.txt',root,beamtype,fieldtype,Linac_Short,Energy_Short,FS_X,FS_Y,D,MU);
        else
            filename = sprintf('%s\\%s\\%s\\Mapcheck\\MLC_%dx%d_%dcm_%dMU.txt',root,beamtype,fieldtype,FS_X,FS_Y,D,MU);
        end
        ...
        if ( filename_long_p == 1 )
            filename = sprintf('%s\\%s\\%s\\Planar Dose\\MLC_%s_%s_%dx%d_%dcm_%dMU',root,beamtype,fieldtype,Linac_Short,Energy_Short,FS_X,FS_Y,D,MU);
        else
...
for d = 1:length(Depth)
    ...
    % Open Mapcheck Data and Compute Profiles
    if ( filename_long_m == 1 )
        filename = sprintf('%s\\%s\\%s\\Mapcheck\\MLC_%s_%s_Banana_%dcm_%dMU.txt',root,beamtype,fieldtype,Linac_Short,Energy_Short,D,MU);
    else
        filename = sprintf('%s\\%s\\%s\\Mapcheck\\MLC_Banana_%dcm_%dMU.txt',root,beamtype,fieldtype,D,MU);
    end
    [ X_m, Y_m, D_m ] = mapcheckTOmat(filename);
    if (rel_analysis == 1)
        d_norm = interp2(X_m, Y_m, D_m, x_offset, y_offset);
...
for r = 0:(length(FieldSize_X)+3)
    ...
    for c = 0:(length(Depth)+2)
        ...
        elseif (r == length(FieldSize_X)+2 && c == 0) || (c == length(Depth)+1 && r == 0) % Average over Points labels
        ...
        elseif (r == length(FieldSize_X)+3 && c == length(Depth)+2) 
            fprintf(fid,'<td>%.1f%%</td>\n',mean(Gamma_Pass_Mean_Depth));
        elseif (r == length(FieldSize_X)+3 && c == 0) || (c == length(Depth)+2 && r == 0) % Average over Percentages labels
            fprintf(fid,'<td><b>Ave. (Percents)</b></td>\n');
        elseif (r == length(FieldSize_X)+3 && c == length(Depth)+1)
            fprintf(fid,'<td>--</td>\n');
