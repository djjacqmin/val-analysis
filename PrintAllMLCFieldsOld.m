...
fieldtype = 'Trans 1p98'; % '05 - MLC Fields'; %'01 - Symmetric Fields\\';
...
for f = 1:length(FieldSize_X)
    ...
    for d = 1:length(Depth)
        ...
        if ( filename_long_p == 1 )
            ...
            filename = sprintf('%s\\%s\\%s\\Planar Dose\\MLC_%dx%d_20x20_%dcm_%dMU',root,beamtype,fieldtype,FS_X,FS_Y,D,MU);
        end
