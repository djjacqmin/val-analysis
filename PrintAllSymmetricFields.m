...
% No Wedge
FieldSize_X = [ 2 3 5 8 10 15 20 25 ]; % cm 
FieldSize_Y = [ 2 3 5 8 10 15 20 25 ]; % cm

% Wedges
% FieldSize_X = [ 2 3 5 8 10 15 15 15 ]; % cm 
% FieldSize_Y = [ 2 3 5 8 10 15 20 25 ]; % cm
...
Linac_Short =  'eX91';  % 'eX91'; % 'iX'; % 
Energy = '16 MV'; % '16 MV'; %
Energy_Short = '16x'; % '16x'; %

root = 'W:\\Private\\Physics\\21eX91 Validation - DJJ';
beamtype =  'Alt MLC Models'; % '2 - 60-Degree Wedge Photons'; % '1 - Open Field Photons'; %  '2 - 60-Degree Wedge Photons'; % '2 - Wedged Field\\';
fieldtype = 'A 1'; % '01 - Symmetric Fields';
mbeamtype =  '1 - Open Field Photons'; %  '2 - 60-Degree Wedge Photons'; % '2 - Wedged Field\\';
mfieldtype = '01 - Symmetric Fields';
...
dtaThreshold = 0.2; % Distance-to-agreement limit in cm
ddThreshold = 0.02; % Dose difference limit as a fraction of maximum dose
...
Gamma_Map_Points = zeros(length(FieldSize_X),length(Depth));
Gamma_Map_Passing_Points = zeros(length(FieldSize_X),length(Depth));
...
for f = 1:length(FieldSize_X)
    ...
    for d = 1:length(Depth)
        ...
        if ( filename_long_m == 1 )
            filename = sprintf('%s\\%s\\%s\\Mapcheck\\%s_%s_%dx%d_%dcm_%dMU.txt',root,mbeamtype,mfieldtype,Linac_Short,Energy_Short,FS_X,FS_Y,D,MU);
        else
            filename = sprintf('%s\\%s\\%s\\Mapcheck\\%dx%d_%dcm_%dMU.txt',root,mbeamtype,mfieldtype,FS_X,FS_Y,D,MU);
        end
        ...
        n = 10; % Increase by a factor of n
        [r, c] = size(D_p);
...
%             imagesc(D_p);
%             plot(1:length(Y_p), length(Y_p)/2,'-w',length(X_p)/2,1:length(Y_p),'-w');
