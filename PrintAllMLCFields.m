...
fieldtype = 'AILL 0p012'; % '05 - MLC Fields'; % '01 - Symmetric Fields\\';
...
Gamma_Map_Points = zeros(length(FieldSize_X)+2,length(Depth));
Gamma_Map_Passing_Points = zeros(length(FieldSize_X)+2,length(Depth));

Gamma_Map_Points_ST = zeros(1,length(Depth));
Gamma_Map_Passing_Points_ST = zeros(1,length(Depth));
...
for d = 1:length(Depth)
    ...
    Gamma_Map_Points_ST(length(FieldSize_X)+2,d) = gc;
    Gamma_Map_Passing_Points_ST(length(FieldSize_X)+2,d) = gp;
...
for r = 0:(length(FieldSize_X)+4)
    ...
    for c = 0:(length(Depth)+2)
        ...
        elseif (r == length(FieldSize_X)+3 && c == 0) || (c == length(Depth)+1 && r == 0) % Average over Points labels
        ...
        elseif (r == length(FieldSize_X)+4 && c == 0) || (c == length(Depth)+2 && r == 0) % Average over Percentages labels
